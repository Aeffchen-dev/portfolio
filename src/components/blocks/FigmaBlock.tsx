...
import { FigmaBlock as IFigmaBlock } from "@/types/blocks";
import { useState } from "react";

const ASPECT_RATIOS: Record<NonNullable<IFigmaBlock["aspectRatio"]>, number> = {
  "16/9": 9 / 16,
  "4/3": 3 / 4,
  "1/1": 1,
  "3/4": 4 / 3,
...
export function FigmaBlock({ block }: { block: IFigmaBlock }) {
  const [loaded, setLoaded] = useState(false);
  const paddingBottom = block.aspectRatio
    ? ASPECT_RATIOS[block.aspectRatio] * 100
    : (9 / 16) * 100;

  return (
    <div className="block-wrapper">
      <div style={{ borderRadius: 16, overflow: "hidden", backgroundColor: "#1a1a1a" }}>
        {/* Figma embed header */}
        <div
        ...
        >
          ...
          <span className="text-xs font-medium truncate" style={{ color: "#888888" }}>
            {block.title ?? "Figma Prototype"}
          </span>
        </div>
